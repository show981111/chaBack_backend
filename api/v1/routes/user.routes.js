...
/*로그인 => 토큰 발급*/
router.post('/login',
    check('userID').notEmpty().isEmail().withMessage('userID should be an Email and not Empty'),
    check('userPassword').notEmpty().withMessage('userPassword should not empty'),
    checkValidationResult,
    userController.login)
/*회원가입*/
router.post('/register',
    check('userID').notEmpty().isEmail().withMessage('userID should be an Email and not be Empty'),
